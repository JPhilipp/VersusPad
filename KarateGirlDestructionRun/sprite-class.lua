...
elseif displayType == 'text' then
    if app.isRetina == nil then app.isRetina = appGetIsRetinaPad() end
...
elseif displayType == 'line' then
    ...
    if self.width ~= nil then self.width = widthOrRadius end
    if self.height ~= nil then self.height = heightOrNil end
...
-- for universal pseudo-spritesheets
self.frameImage = nil
self.frameImageOld = nil
self.frameImageZ = nil -- or 'background'
...
function self:adjustFrameImage()
    if self.frameImage ~= nil then
        if self.frameImage ~= self.frameImageOld then
            local oldFrameSprite = appGetSpriteByTypeAndParentId('frame', self.id)
            if oldFrameSprite ~= nil then oldFrameSprite.gone = true; oldFrameSprite.isVisible = false end
    
            local width = self.width; local height = self.height
            if self.displayType == 'circle' then
                width = self.radius * 2
                height = width
            end
            local frame = spriteModule.spriteClass('rectangle', 'frame', self.type, self.frameImage, false, self.x, self.y, width, height)
            frame.parentId = self.id
            frame.movesWithParent = true
            frame.rotatesWithParent = true
            frame.rotation = self.rotation
            frame.doDieOutsideField = false
            if self.alpha ~= nil and self.alpha ~= 1 then frame.alpha = self.alpha end
            if self.frameImageZ == 'background' then
                frame:toBack()
                if self.type ~= 'background' then
                    local background = appGetSpriteByType('background')
                    if background.frameImage ~= nil then background = appGetSpriteByType('frame', 'background') end
                    if background ~= nil then background:toBack() end
                end
            end
            appAddSprite(frame)
    
            self.frameImageOld = self.frameImage

        elseif self.alpha ~= nil and self.alpha ~= 1 and self.alphaChangesWithEnergy and self.energySpeed ~= 0 then
            local frameSprite = appGetSpriteByTypeAndParentId('frame', self.id)
            if frameSprite ~= nil then frameSprite.alpha = self.alpha end
...
function self:rotateWithParentIfNeeded()
    if self.rotatesWithParent then
        local parentSprite = appGetSpriteById(self.parentId)
        if parentSprite ~= nil then
