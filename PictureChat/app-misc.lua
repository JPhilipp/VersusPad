...
function appPlaySound(soundName, optionalChannel)
...
function appPlaySoundForced(soundName)
    if string.find(soundName, '%.') == nil then soundName = soundName .. '.mp3' end
    soundName = appToPath(soundName)
    local soundHandle = audio.loadSound(soundName)
...
function appPrint(stringOrStringTable, forcePrint, doIndent, showGuid)
    ...
    if doIndent == nil then doIndent = true end
    if showGuid == nil then showGuid = true end
...
function appCreateDebug(x, y, width, height, fontSize, colorTriple, alpha)
    if app.showDebugInfo then
        ...
        if colorTriple == nil then colorTriple = {red = 0, green = 0, blue = 0} end

        local self = spriteModule.spriteClass('text', 'debugText', nil, nil, false, x, y, width, height)
        self.size = fontSize
        self:setRgbByColorTriple(colorTriple)
        self.alpha = alpha
        self:setColorBySelf()
        self:toFront()
        appAddSprite(self)
...
function appCreateClock(x, y, width, height, fontSize, colorTriple, alpha)
    if app.showClock then
        if x == nil then x = app.maxXHalf end
    ...
    if app.isAndroid then
        ...
        if app.maxX > app.maxY then
            for n = 1, 2 do
                ...
                if image == nil then self:setRgbByColorTriple(app.letterboxColor[n]) end
