...
function spriteClass(displayType, type, subtype, imageName, isAutoPhysical, x, y, widthOrRadius, heightOrNil,
            framesData, polygonShape, parentPlayer, isSensor, density, bounce, friction)
local self = {}
...
density = nil; bounce = nil; friction = nil
...
if self.isAutoPhysical then
    if self.displayType == 'circle' then
        ...
        physics.addBody( self, { isSensor = isSensor, density = density, friction = friction, bounce = bounce, shape = polygonShape, filter = appGetCollisionFilter(self) } )
    end
end
...
function self:setFillColorBySelf()
    ...
    if self.displayType ~= 'text' and colorChanged then
