...
function getIsoDate(date)
    ...
    if false and app.isLocalTest then
        -- isoDate = '2009-01-09'
        -- appPrint('Changing date to ' .. isoDate)
    end
    return isoDate
end
...
function getRandomEntry(table)
    local v = nil
    if type(table) == 'table' and #table >= 1 then
        if #table == 1 then
            v = table[1]
        else
            v = table[ math.random(1, #table) ]
        end
    end
    return v
end
...
function printTable(list, i)
    local listString = ''
    if not i then listString = listString .. '{' end

    i = i or 1
    local element = list[i]

    if not element then return listString .. '}' end

    if type(element) == 'table' then
        listString = listString .. misc.printTable(element)
    else
