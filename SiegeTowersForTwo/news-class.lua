...
function self:handle()
    ...
    -- appPrint( 'waitBeforeTryShowingDays = ' .. tostring(waitBeforeTryShowingDays) )
    ...
    if lastTriedToShowDate == nil or waitBeforeTryShowingDays == nil then
        ...
        -- appPrint( 'last ' .. tostring(lastTriedToShowDate) .. ' | wait ' .. tostring(waitBeforeTryShowingDays) .. ' | diff ' .. tostring(differenceInDays) )

        if tonumber(differenceInDays) >= tonumber(waitBeforeTryShowingDays) then
            -- appPrint('Contacting server...')
            app.data:set('dialog_lastTriedToShowDate', today)
            app.data:set( 'dialog_waitBeforeTryShowingDays', math.random(1, 2) )

            local doOfflineTest = false and app.isLocalTest
            if doOfflineTest then
                local testResponse = 'nervousFlyTest1;citybucks,dreamplace,nervousfly;;Have you seen Foobar? It is a bar with a foo and lots of barfoos too :);' ..
                        'itms://itunes.apple.com/us/app/foobar/id384664859;1'
                app.news:handleNewsResponse(testResponse)
            else
...
function self:grabAndHandleLiveNews()
    ...
    local newsUrl = 'http://file.versuspad.com/news.txt'
    if app.device ~= 'iPad' then newsUrl = 'http://file.versuspad.com/news-android.txt' end
    newsUrl = newsUrl .. '?guid=id' .. misc.getRandomString(32)
    newsUrl = newsUrl .. '&supportsUnicode=true&app=' .. app.name .. '&v=' .. app.version
    print(newsUrl)
...
function self:show(guid, text, url)
    ...
    if app.gameRuns then appPauseGame() end
...
function self:approveAndGoTo()
    ...
    timer.performWithDelay(3000, appResumeGame)
end
...
function self:verifyNeededFunctionsAndVariablesExists()
    ...
    if app.gameRuns == nil then appPrint(preceder .. 'app.gameRuns', true) end
    if appPauseGame == nil then appPrint(preceder .. 'appPauseGame', true) end
    ...
    if appResumeGame == nil then appPrint(preceder .. 'appResumeGame', true) end
