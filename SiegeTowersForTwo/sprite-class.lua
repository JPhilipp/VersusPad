...
function spriteClass(displayType, type, subtype, imageName, isAutoPhysical, xOrLinePointsArray, y, widthOrRadius, heightOrNil,
        framesData, polygonShapeOrShapes, parentPlayer, isSensor, density, bounce, friction)
local self = {}
...
if imageName ~= nil then
    ...
    if framesData ~= nil then
        ...
        local options = {sheetContentWidth = framesData.image.width, sheetContentHeight = framesData.image.height, numFrames = framesData.image.count,
                width = widthOrRadius, height = heightOrNil}
...
function self:setFillColorBySelf()
    ...
    if colorChanged then
        ...
        if self.displayType == 'text' then
            self:setTextColor(self.red / 255, self.green / 255, self.blue / 255, self.alpha * 255)
        elseif self.displayType == 'line' then
            self:setColor(self.red / 255, self.green / 255, self.blue / 255, self.alpha * 255)
        else
            self:setFillColor(self.red / 255, self.green / 255, self.blue / 255, self.alpha * 255)
        end
    end
end
...
function self:setColorBySelf()
    if self.displayType == 'text' then
        self:setTextColor(self.red / 255, self.green / 255, self.blue / 255, self.alpha * 255)
    elseif self.displayType == 'line' then
        self:setStrokeColor(self.red / 255, self.green / 255, self.blue / 255, self.alpha * 255)
    end
end
...
function self:isOutside()
    return (self.xFloat + self.width < app.minX) or
            (self.xFloat - self.width > app.maxX) or
            (self.yFloat + self.height < app.minY) or
            (self.yFloat - self.height > app.maxY)
end
...
function self:adjustRadius(radiusSpeed)
    self.path.radius = self.path.radius + radiusSpeed
    if self.path.radius <= 1 then self.path.radius = 1 end
    self.width = self.path.radius * 2
    self.height = self.path.radius * 2
end
