...
function appStartPurchase(id)
    if store.canMakePurchases then
        ...
        if app.runs then app.phase:set('build')
        else app.menu:createDialogSetParts()
        end
    end
end
    ...
    if state == 'purchased' or state == 'restored' then
        ...
        for i = 1, #app.products do
            ...
            if id == transaction.productIdentifier or id == transaction.originalIdentifier or id == transaction.originalTransactionIdentifier then
                if not appGetProductIsPurchasedPerDB(id) then
                    app.gold = app.gold + product.goldAmount
                    product.isPurchased = true
                    appSavePurchaseInDB(id)
                    appSaveData()

                    if state == 'purchased' then
                        appAlert( 'Congratulations, you received ' .. misc.addCommasToNumber(product.goldAmount) .. ' gold!' )
                    elseif state == 'restored' then
                        appAlert( 'Successfully restored ' .. product.title .. ' and added ' .. tostring(product.goldAmount) .. ' gold!' )
                    end
                    appPlaySound('gold')
                end
                somethingFound = true
        ...
        if not somethingFound then
            if state == 'restored' then
                appAlert('No past gold purchases found to restore.')
            else
